...
def randomForests(trainingSet,testingSet, max_depth, n_trees):
    ...
    def decision_tree_algorithm(df, counter = 0, min_samples = 50, max_depth = max_depth, random_subspace = random_subspace):
        ...
        if (unique_label_func(data)) or (len(data) <= min_samples) or (counter == max_depth):
            ...
            yes_answer = decision_tree_algorithm(data_above, counter, min_samples = 50, max_depth = max_depth, random_subspace = random_subspace)
            no_answer = decision_tree_algorithm(data_below, counter, min_samples = 50, max_depth = max_depth, random_subspace = random_subspace)
    ...
    def random_forest_algorithm(train_df, random_subspace,n_trees = n_trees, max_depth = max_depth):
        ...
        for i in range(n_trees):
            df_boostrapped = bootstrapping(train_df)
            tree = decision_tree_algorithm(df_boostrapped, min_samples = 50, max_depth = max_depth, random_subspace = random_subspace)
            forest.append(tree)
        return forest
...
# # Influence of Number of Trees on Classfier Performance
...
Bagging_average = []
randomforest_average = []

Bagging_standarderror = []
randomforest_standarderror = []
...
for j in n_tree:
    ...
    for i in range(len(folds)):
        ...
        acc_BA = bagging(train_set, test_set, max_depth = 8, n_trees = j)
        acc_RF  = randomForests(train_set,test_set, n_trees = j, max_depth = 8)
        

        baggedtree.append(acc_BA)
        randomforest.append(acc_RF)
    
    print("Accuracies For Bagged Decision Tree with trees = " + str(j))
    print(baggedtree)
    
    print("Accuracies For Random Forest with trees = " + str(j))
    print(randomforest)
    ...
    Bagging_average.append(Avg_accuracy_BA)
        
    Standard_error_BA = (np.std(baggedtree)/np.sqrt(10))
    Bagging_standarderror.append(Standard_error_BA)
    
    Avg_accuracy_RF = np.mean(randomforest)
    randomforest_average.append(Avg_accuracy_RF)
    
    Standard_error_RF = (np.std(randomforest)/np.sqrt(10))
    randomforest_standarderror.append(Standard_error_RF)

print("\n")
print("Average accuracy accross 10 folds for Bagged Decision Tree: " + str(Bagging_average))
print("Average Standard error accross 10 folds for Bagged Decision Tree: " + str(Bagging_standarderror))

print("\n")
print("Average accuracy accross 10 folds for Random Forest: " + str(randomforest_average))
print("Average Standard error accross 10 folds for Random Forest: " + str(randomforest_standarderror))
...
BA = plt.errorbar( n_tree, Bagging_average, yerr = Bagging_standarderror, fmt = '--o', ms = 10 , mew=4, marker = 'd')
RF = plt.errorbar( n_tree, randomforest_average, yerr = randomforest_standarderror , fmt = '--o', ms = 10 , mew=4)
plt.xlabel("Number of trees")
plt.ylabel("Accuracy")
plt.title("Mean Accuarcy vc Number of Trees ")
plt.legend((BA, RF), ('Bagged Decision Tree', 'Random Forest'), bbox_to_anchor=(0.73, 0.12), loc=2, borderaxespad=0.1)
plt.show()


# Hypothesis Testing: Performance between Random Forest and Bagged Tree accross number of trees
...
scipy.stats.ttest_ind([0.64, 0.74, 0.74, 0.73, 0.71, 0.78, 0.64, 0.68, 0.71, 0.68] , [0.66, 0.73, 0.72, 0.69, 0.73, 0.77, 0.67, 0.64, 0.7, 0.68])
...
scipy.stats.ttest_ind([0.65, 0.73, 0.75, 0.74, 0.74, 0.77, 0.65, 0.67, 0.72, 0.69] , [0.7, 0.75, 0.75, 0.72, 0.73, 0.79, 0.67, 0.69, 0.72, 0.67])
...
scipy.stats.ttest_ind( [0.69, 0.76, 0.74, 0.74, 0.72, 0.78, 0.65, 0.69, 0.72, 0.7],[0.7, 0.76, 0.74, 0.72, 0.76, 0.79, 0.7, 0.7, 0.72, 0.7] )
...
scipy.stats.ttest_ind([0.68, 0.77, 0.76, 0.74, 0.74, 0.8, 0.65, 0.7, 0.73, 0.7] , [0.71, 0.77, 0.78, 0.74, 0.77, 0.79, 0.69, 0.69, 0.72, 0.7])
